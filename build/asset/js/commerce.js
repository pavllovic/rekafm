...
RangeBarInt.prototype = {
  ...
  passValueFrom: function passValueFrom() {
    var value = (this.valueFrom - this.valueMin) / (this.valueMax - this.valueMin) * 100;
    this.wrapper.style.setProperty('--value-from', "".concat(value, "%"));
  },
  passValueTo: function passValueTo() {
    var value = (this.valueTo - this.valueMin) / (this.valueMax - this.valueMin) * 100;
...
MultiCombobox.prototype = {
  ...
  reset: Lib_multiCombobox_multiCombobox_js__WEBPACK_IMPORTED_MODULE_0__.reset,
  ...
  handleEvent: function handleEvent(e) {
    switch (e.type) {
      case 'click':
        ...
        if (e.target === this.btnClose) return this.hideFilter(); // if(e.target.getAttribute('data-filter') === 'reset') return this.hideFilter();
...
/***/ "./asset/components/filter/FilterActions.js":
/*!**************************************************!*\
  !*** ./asset/components/filter/FilterActions.js ***!
  \**************************************************/
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
...
var FilterActions = function FilterActions(filter) {
  this.filter = filter;
...
FilterActions.prototype = {
  ...
  setListeners: function setListeners() {
    ...
    this.btnReset.addEventListener('click', function () {
      return _this.hideBtnReset();
    });
    this.btnApply.addEventListener('click', function () {
      return _this.showBtnReset();
    });
  },
  showBtnReset: function showBtnReset() {
...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = (FilterActions);
...
TablistView.prototype = {
  ...
  hideFilterBar: function hideFilterBar() {
    ...
    setTimeout(function () {
      _this2.filterBar.classList.remove('fixed');
    }, 1000);
  },
  showFilterBar: function showFilterBar() {
    ...
    setTimeout(function () {
      _this3.filterBar.classList.add('fixed');
    }, 1000);
  },
...
function updateOutput() {
  ...
  if (this.mapSelected.size > 1) {
    output = "".concat(Array.from(this.mapSelected)[0][1].value, ",..(").concat(this.mapSelected.size, ")");
  }
...
function init() {
  ...
  var selector = urlParams.get('tab') ? "#".concat(urlParams.get('tab')) : '[role="tab"]';
  this.activeTabId = 'tab-buying';
  this.tablists.forEach(function (item) {
    _this.activateTab(item.querySelector("".concat(selector)));
  });
...
function activateTab(tab) {
  ...
  var newUrl = "".concat(window.location.origin).concat(window.location.pathname, "?").concat(str);
...
var initRangeBar = function initRangeBar() {
  ...
  if (rangeBarFloatElements.length > 0) {
    rangeBarFloatElements.forEach(function (element) {
      var rangeBar = new _components_UI_ui_rangeBar_rangeBar__WEBPACK_IMPORTED_MODULE_13__["default"](element, 'float');
      rangeBar.init();
      var filterName = element.closest('[data-filter]').getAttribute('data-filter');
      var filter = window.filters.get(filterName);
      filter.controls.push(rangeBar);
...
window.map.createObjectCollection(); // window.map.addObjectsInObjectCollection([a, b]);

var initCommerceControls = function initCommerceControls() {
  initFilterActions();
  initFilterSelects();
  initFilterMultiSelects();
  initRangeBar();
